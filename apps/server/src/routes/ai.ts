...
/** Generate an IELTS Listening exercise + fill-in questions.
 *  Body: { section?: 1 | 4 } — 1 = Section-1 conversation, 4 = Section-4 lecture. */
aiRoutes.post('/listening', async (c) => {
  if (!API_KEY) return c.json({ error: 'AI 未配置（缺少 AI_API_KEY）' }, 503);
  const body = await c.req.json().catch(() => ({}));
  const section = body?.section === 4 ? 4 : 1;
  ...
  const lecturePrompt =
    `写一段**原创**的雅思听力 Section 4 风格英文学术讲座（单人独白，一位讲师），主题学术但通俗（历史/环境/心理/科技/生物/社会等，自选其一），约 220-280 词，自然分成 3-4 段。内容自拟、真实感强，不得抄袭现实材料。` +
    `然后基于讲座出 6 道笔记/摘要补全填空题：每题是一句带一个空 "_____" 的笔记，答案必须是讲座中**原词**出现的一个词或很短的词组（1-3 个词，如术语/名称/数字/年份等），大小写不敏感。` +
    `每题附简短中文解析（说明答案来自哪句）。只返回 JSON，不要 markdown（lines 用讲座的每一段，speaker 都填 "Lecturer"）：` +
    `{"title":"...","scenario":"中文一句话讲座主题","lines":[{"speaker":"Lecturer","text":"第一段..."},{"speaker":"Lecturer","text":"第二段..."}],` +
    `"questions":[{"q":"Photosynthesis mainly happens in the _____.","answer":"chloroplast","explain":"..."}]}`;

  const prompt = section === 4 ? lecturePrompt : conversationPrompt;
